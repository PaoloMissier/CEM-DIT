...
\jwb{We argue here that abstraction is the usual word in this context (PROV) for this work. }

\jwb{In Zoom~\cite{DBLP:conf/icde/BitonBDH08} an abstraction mechanism is a mechanism that presents the most relevant provenance information to a user. }

\jwb{It is used in~\cite{Moreau2015}, in which the author discusses a variety of means of abstracting provenance graphs, including~\cite{Missier2014}, in which the term carries the same meaning as in this work.  Also discussed are the Abstract Provenance Graphs from~\cite{ZinnLudascher2010}. In these, abstraction is performed using static analysis of a configured workflow before the provenance is created, and a homomorphism between the abstract and the concrete graph is demonstrated. Although they create their abstraction in a different way, the same effect, of removing unnecessary information, is achieved. }
%
\jwb{Finally, ~\cite{Moreau2015} discusses~\cite{Cohen-Boulakiaetal2008}, in which the authors show how the user views that they present allow provenance to be reasoned about at different levels of abstraction. }
\jwb{This understanding of abstraction, and of the ability to move between different levels of abstraction, is key to our understanding of the term here. }

<<<<<<< HEAD
\jwb{ Say abstraction is the usual word in this context (PROV) for this work back up with references. (eg)  The second reviewer (more experienced in PROV?) has no difficulty with the term. I suggest we include a brief discussion on terminology somewhere that makes reviewer ones points.} 
=======
\jwb{Finally, the second reviewer has offered no objection to the term.} 
...
>>>>>>> 4dbbd56d55bd3a3b98a618ab77a90a96ff692454
...
\jwb{The grouping operator is defined functionally, using the three operators path closure, extend and replace. We have added text to clarify this in the first paragraph of sec 1.2 and in the introduction text of section 4.}

\item There are several issues with Fig 6. 

\jwb{Fig 6. was an example of the problems that a naive approach would face. It is clear to us now that the approach itself was problematic for more than one reviewer, and so section 4 has been substantially revised to remove a misleading emphasis on wrong solutions, and move more quickly to the solution that we present. This, we trust, deals with the fundamental issue behind many of the reviewer's points below.}
...
\jwb{We feel it would better aid comprehension to remove  figure 6.}
...
\jwb{The group operator as defined later in section 4 will not produce a false a dependency here. }
...
%\comment{is this assumption (that agent nodes are disjoint from entity and activity nodes) not always true? PM to clarify.}
...
\item Discuss alternative abstraction strategies such as using wasInfluencedBy edges.
  ...
  \comment{wIb might relax the need for convexity, so we can save more nodes, but the loss on information in the ``lossy'' wIb.}
  
  \paolo{the argument here is that there may be a trade-off between a ``convex'' abstraction that preserves the semantics of the relationships but may reduce the residual utility, and an abstraction that simply maps relationships that ``don't work'' to WasInfluencedBy without trying to abstrat nodes that were not intended to be abstracted.
  In practice we need an example to show how WasInfluencedBy would work and whether this trade-off is real}
...
\comment{must rework conclusion completely}
...
\jwb{Decision is to drop the (less mature) work on agents} \comment{Need to implement this decision in paper}
