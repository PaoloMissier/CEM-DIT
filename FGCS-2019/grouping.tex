...
\jwbtwo{\subsection{Justifying relations}} \label{sec:justifying}
...
\jwbtwo{To see that the $\group$ operator produces justified abstract graphs, consider two graphs: a concrete one, $C$, represented by the graph $G_{C} = (V_C,E_C)$, and an abstract one $A$, represented by the graph $G_{A} = (V_A,E_A)$.
If $(v^A_d \xleftarrow{t} v^A_s)$ is a relation in $E_A$ of type $t$,  $(v^A_d \xleftarrow{t} v^A_s)$ in $E_A$ we need to show that there is a relation $(v^C_{d'} \xleftarrow{t'} v^C_{s'})$ in $E_C$ such that either (i) the relation is unchanged, so 
...
In all cases the type of the relation should be unchanged. }
\end{definition}
...
\jwbtwo{
Considering (i) first, we need to show that the type of the relation remains unchanged during the abstraction operation. But if $v^A_s$ and $v^A_d$ have not changed, then, by Definition~\ref{def:in-out-int}, $v^A_s \nin \vartheta_{out}(V^*)$, $v^A_s \nin \vartheta_{in}(V^*)$, and $v^A_s \nin \vartheta_{int}(V^*)$  and so, by Definition~\ref{def:group-replace} neither is removed from $E_C$ in the transformation by $\group$ to $E_A$, and so the type of the relation does not change, so $t=t'$. 
}
\jwbtwo{
Considering (ii), $v^A_d$ is the new node $v_{new}$ and $v^A_s$ is unchanged. In this case, to justify the new relation, we need to show that there is a relation $(v^C_{d'} \xleftarrow{t'} v^C_{s'})$  in $E_C$, and that the operation of $\group$ produces the relation $(v_{new} \xleftarrow{t} v^A_{s})$ in $E_A$, where $V^A_s = V^C_{s'}$ and $t=t'$. 
%
We see this by considering the definition of $\vartheta_{in}(V^*)$ in Definition~\ref{def:in-out-int}. The source of the relation is unchanged, since $v_s \in V\setminus V^*$, so it is not in the set $V^*$ that has been chosen to be abstracted. The replacement relation is given by the definition of $\vartheta'_{in}(V^*)$ in Definition~\ref{def:var-in-out}, from which we see that the source and type of the relation are unchanged in $E_A$.
}
