...
\comment{Fig 6. was an example of the problems that a naive approach would face. It is clear to us now that the approach itself was problematic for more than one reviewer, and so section 4 has been substantially revised to remove the emphasis on wrong solutions, and move more quickly to the solution that we present. This, we trust, deals with the fundamental issue behind the reviewer's points below.}
...
\comment{"rewired" is a poor choice of term and has been removed. The labels for edges in each of the diagrams in the paper are now clear.} 
...
\comment{We feel it would better aid comprehension to remove  figure 6.}
...
\comment{'Group' returns a PROV graph, and is a rewriting operator. We have clarified this at the start of section 1.2.  Rather than being an alternative to path closure, replace and extend, Group  is defined using these operators.  The rewiring is part of the definition of group, defined in the subordinate operator replace.  }  \jwb{Should we sprinkle the word "rewriting" about? would that help?}
...
\item There is something wrong with the mathematical definition: $PG_{gu/ea}  ~~X~~ P(V) \rightarrow PG_{gu/ea}$
 ...
 \comment{We've clarified that this equation is the type signature of the group operator, rather than it's definition.}
 
 \comment{The powerset operator has been named when it is  used.}
 
 \comment{the equation has been numbered.} 
...
\comment{The definition of Group is in terms of extend, closure and replace. The edges are dealt with by the replace operator.}
...
\comment{t is the type of the nodes on the boundary that Extend creates. We have added tet just above the definition of Extend to clarify this.}

\item Figure 8 uses a new keyword ’absorb’. Why isn’t it ‘extend’ now?

\comment{The figure has been corrected.}   

\item How is Definition 6 related to the grouping operator? Is it related?

The significance of this definition is not clear. 

\comment{Defn 6 is the definition of the Group operator under the assumption that all the nodes removed are of the same type (homogeneous grouping). We make this clear at the start of section 4.1, and again just before  definition 6. Section 4.2 generalises to full grouping. }



\item In Section 4.2 the word propagation is used. Is it similar to extension? 

  \comment{We have avoided the word propagation and clarified that strict $t_grouping$ involves a further step. } 
