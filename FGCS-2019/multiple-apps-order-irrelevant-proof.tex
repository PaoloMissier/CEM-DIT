...
%For readability, we introduce simplifying definitions throughout the proof. 
...
%  \INN(v_{N1}) = \{(v',v_{N1},r(l))|(v',v,l) \in in(v), v\in V_a\} \\
%  out(v_{N1})= \{(v_{N1},v',r(l))|(v,v',l) \in out(v), v\in P_1\} \\
% \]
% \noindent
%With a slight abuse of notation, $\INN(\{v_{N1}\})$ (resp. $\OUT(\{v_{N1}\}), \INT(\{v_{N1}\})$)  is written $\INN(v_{N1})$ (resp. $\OUT(v_{N1}), \INT(\{v_{N1}\})$). These simplifies the definition of $E_1$ to 
%\[
 % E_1  = ( E\hide(\INN(P_1) \union \OUT(P_1)) \union \INT(P_1)) \union ( \INN(v_{N1}) \union \OUT(v_{N1}) )
%\]
\noindent  
...
Next, we expand the definition of $\INN(v_f)$. Recall \comment{hit the buffers here; continue later}
