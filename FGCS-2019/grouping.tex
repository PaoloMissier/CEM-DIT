...
%
\comment{need to be clear (again) about assumptions. We have motivated these in Sec~\ref{sec:motivation}. } 
...
%\jwbtwo{Several graph editing options are possible, depending on the information we want to remove. We may remove chosen parameters from nodes or to anonymise individual nodes, in each case retaining the original structure of the graph.}
%
\jwbtwo{We require the final operator to remove all the chosen nodes, and replace them by a single abstract node.} \jwbtwo{As discussed in Section~\ref{sec:motivating}, this may be to avoid inadvertently revealing any internal business processes or details about the chain of command.}
%\
...
%
$\group$ takes a graph $G = (V, E) \in \guEA$ and a subset $V_{gr} \subset V$ of its nodes \jwb{that the user wishes to obfuscate} and produces a modified graph $G' \in \guEA$. The nodes in $V_{gr}$ are ``grouped'' together and replaced by a new single node. \jwbtwo{The $\group$ operator has the following signature:}
...
\jwbtwo{Below, we take a functional approach to the definition, by defining three subordinate functions and then defining $\group$ as the functional composition of these subordinate functions. }

\jwbtwo{In Section~\ref{sec:closure} we assume that the set of nodes to be removed are all of the same type ($\en$ or $\act$) We refer to this as \emph{homogeneous grouping}
and the replacement node is of the same type as the removed nodes.} 
%
\jwb{In Section~\ref{sec:generalisation} we remove these assumptions, so both entities and activities can be removed. A consequence is that  the type of the replacement node must be  explicitly given. This leads to two variants, depending on which type ($\en$ or $\act$) is chosen for the final replacement node.   }
...
\comment{talk about homogenous grouping as the purpose of this sub section.} 
\comment{motivation: simplification, the more complex case to be treated later.}

\jwbtwo{In this subsection we present a version of the $\group$ operator, that will replace a selected set of nodes with an abstracted one. We first work under the simplifying assumption that all the nodes selected are of the same type (entity).   The general case, where selected nodes can be of either type, is covered in Section~\ref{sec:generalisation}.}

\jwbtwo{We wish to ensure that the final abstracted graph is a valid PROV graph. Let the set of nodes to be replaced in a graph $G$ be $V_{gr}$. An issue, pointed out in the description of the ProPub system~\citep{springerlink:10.1007/978-3-642-22351-8_13}, mentioned earlier, is that removing $V_{gr}$ and replacing it with a single node can lead to cycles in the modified graph. Intuitively, this occurs when $V_{gr}$ is not``convex'', that is, there are paths in the graph that lead out of $V_{gr}$ and then back in again.}
...
\jwbtwo{We assume, for the moment, that nodes in $\clos(V_{gr}, G)$ form a single connected subgraph under $G$, and that each source is connected to at least one of the sinks.} 
...
%
\jwbtwo{Finally, let $\vartheta_{int}(V^*)$ denote internal edges, that connect two nodes inside $V^*$:}
...
\comment{Other assumptions: group homogeneity is discharged here. Each source is connected to a sink? Actually, this isn't a separate assumption. It is just two/more groups.    }
...
\jwbtwo{Fig.~\ref{fig:e2-a4}(a) is a subgraph of the previous example.}
...
The intuitive interpretation for this pattern is that each of the two activities generated one entity in the group represented by $e_{new}$, and that the abstraction makes these two events indistinguishable. \comment{what abstraction? Not ours?} Formally, nothing further needs to be done to the graph. \jwb{We will explore the implications of the event ordering rules further in Section~\ref{sec:event}.  }  However note that one can restore, if desired, the more natural pattern whereby one single generation event is recorded for $e_{new}$. This is achieved simply by propagating the grouping to the set of generating activities. \comment{Involves a renaming to $a_{new}$. I this we should drop this bit and   Fig.~\ref{fig:e2-a4}(e-3)} In the example, this leads to the graph in Fig.~\ref{fig:e2-a4}(e-3).  
...
We now formalize these considerations by introducing two definitions for $\group$. The first, which we call \textbf{t-grouping}, is agnostic of multiple generation patterns, while the second (\textbf{strict t-grouping}) applies propagation to ensure that the graph is free from multiple generation patterns. \comment{does it rename?} \comment{do we refer to e-grouping and a-grouping previously?}
...
\jwbtwo{Strict t-grouping} performs the further step illustrated in the transition from Fig.~\ref{fig:e2-a4}(e-2) to Fig.~\ref{fig:e2-a4}(e-3). 
...
\end{definition}
\jwbtwo{Note that that the $Group$ operator can only produce an multiple generation pattern if $type(v_{new})=En$.}
%
