...
\jwb{Abstraction is the usual word in this context (PROV) for this work. 
  The second reviewer (more experienced in PROV?) has no difficulty with the term.}

\jwb{I suggest we include a brief discussion on terminology somewhere that makes reviewer ones points.} 
...
\jwb{'Group' returns a PROV graph, and is a rewriting operator. We have clarified this at the start of section 1.2.  Rather than being an alternative to path closure, replace and extend, Group  is defined using these operators.  The rewiring is part of the definition of group, defined in the subordinate operator replace.  }  \comment{Should we sprinkle the word "rewriting" about? would that help?}
...
\jwb{The definition of Group is in terms of extend, closure and replace. The edges are dealt with by the replace operator.}
...
\item refers to “issue”. Which one? Previous para states two issues. 
  ...
  \jwb{We have reworked the text in Section 4 to introduce the issues (the introduction of cycles and badly typed relations) separately, and to dealt with them in turn. }
...
\jwb{t is the type of the nodes on the boundary that Extend creates. We have added tet just above the definition of Extend to clarify this.}

\item Figure 8 uses a new keyword ’absorb’. Why isn’t it ‘extend’ now?

\jwb{The figure has been corrected.}   
...
\item In Section 4.2 the word propagation is used. Is it similar to extension? 
  ...
  \jwb{We have avoided the word propagation and clarified that strict $t\_grouping$ involves a further step. } 
