...
\jwbtwo{In this section, we clarify what it means to say that relations in the abstract graph are justified, and show that the relations produced by the $\group$ (more properly, the family of $\group$ operators) are justified.}
 

\begin{definition}
\label{def:justify}
\jwbtwo{Consider two graphs: a concrete one, $C$, represented by the graph $G_{C} = (V_C,E_C)$, and an abstract one $A$, represented by the graph $G_{A} = (V_A,E_A)$.
If $(v^A_d \xleftarrow{t} v^A_s)$ is a relation in $E_A$ of type $t$,  $(v^A_d \xleftarrow{t} v^A_s)$ in $E_A$ is justified if there is a relation $(v^C_{d'} \xleftarrow{t'} v^C_{s'})$ in $E_C$ such that either (i) the relation is unchanged, so 
$v^A_d = v^C_{d'}, t=t'$ and $v^A_s = v^C_{s'}$;
(ii) either the source of the destination node is a new (and therefore abstract) node which the $Group$ operator has inserted as a replacement for a set of nodes, and the other node is unchanged.
In all cases the type of the relation should be unchanged. }
\end{definition}

\jwbtwo{To see that relations are justified in this sense, consider the $\group$ operator. In particular, inspection of definitions~\ref{def:var-in-out} and~\ref{def:group-replace} shows that }
