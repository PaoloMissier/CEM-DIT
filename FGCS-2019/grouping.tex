...
%
\jwb{In Section~\ref{sec:generalisation} we remove these assumptions, so entities and activities can be removed by the same application of $\group$. A consequence is that the type of the replacement node must be  explicitly given. This leads to two variants of the operator,  depending on which type ($\en$ or $\act$) is chosen for the final replacement node.   }
...
\jwbtwo{We} now provide an initial definition of our $\group()$ operator, under the simplifying assumption that all nodes in $V_{gr}$ are of the same type \jwb{before closure.}  We denote this type by $\type(V_{gr})$ (with a slight abuse of notation), and denote the initial definition of $Group$ as $\group_{hom}$ \jwb{Definitions~\ref{def:t-grouping} and~\ref{def:strict-t-grouping}  in Section~\ref{sec:generalisation} remove this assumption.}
...
We now formalize these considerations by introducing two definitions for $\group$. The first, which we call \textbf{t-grouping}, is agnostic of multiple generation patterns, while the second (\textbf{strict t-grouping}) applies \jwbtwo{a further step} to ensure that the graph is free from multiple generation patterns. \comment{do we refer to e-grouping and a-grouping previously?}
